...
export async function generateOutline(
  ...
  if (!isRecord(parsed) || !isRecord(parsed.outline)) {
    throw new ResearchServiceError('AI_GENERATION_RESPONSE_INVALID', 502, 'AI 返回的大纲结构无效。')
  }
  ...
  if (!title || title.length > 200 || rawSections.length < 2 || rawSections.length > 10) {
    throw new ResearchServiceError('AI_GENERATION_RESPONSE_INVALID', 502, 'AI 返回的大纲标题或章节数量异常。')
  }

  const seenTitles = new Set<string>()
  const sections = rawSections.map((item, index) => {
    if (!isRecord(item)) {
      throw new ResearchServiceError('AI_GENERATION_RESPONSE_INVALID', 502, 'AI 返回了无效大纲章节。')
    }
    ...
    if (
      ...
      throw new ResearchServiceError('AI_GENERATION_RESPONSE_INVALID', 502, 'AI 返回了重复或过长的大纲章节。')
    }
    ...
    if (sourceIds.some((sourceId) => !allowedSourceIds.has(sourceId))) {
      throw new ResearchServiceError('AI_GENERATION_RESPONSE_INVALID', 502, 'AI 大纲包含资料池之外的来源。')
    }
